...
## Rationale
Producers that emit cumulative input history on every chat span re-stamp the entire conversation each turn — re-rendering the carried-forward prefix dominates the byte view and buries the new content, so the suffix optimization elides it. Producers that emit per-turn input deltas share no message ids with the prior chat, so the suffix optimization correctly no-ops there (`prefixLen === 0`). Preserving the original-array index in suffix node ids keeps external cross-links (e.g., ChatDetail's tool-call arrow targeting a `tool_call_response`) stable regardless of whether the optimization fires.
