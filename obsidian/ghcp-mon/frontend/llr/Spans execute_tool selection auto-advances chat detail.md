...
## Rationale
OTel GenAI semconv permits two valid tool-span hierarchies and the producers we observe use different ones. **Sibling shape** — tool spans run as siblings of a still-running chat span which then consumes the response. **Nested shape** — tool spans nest under a chat span and the response surfaces in the next chat span globally. Branching on tree shape rather than `service_name` keeps the routing robust to mixed traces.
