...
## Rationale
OTel GenAI semconv permits two valid tool-span hierarchies, and the two producers we observe use different ones. Copilot CLI (agent-loop-level instrumentation) dispatches tool spans as siblings of a still-running chat span and appends the response to that same chat's input.messages — so the response-consuming chat is the sibling whose lifetime encloses the tool. opencode (SDK-level instrumentation) nests tool spans under a chat span and surfaces the response in the *next* chat span globally (the one that fires after the parent chat completes and the loop iterates). Branching on tree shape rather than vendor name makes the routing robust to mixed traces.
